...
data = data.assign(intelligence=0, strength=0, power=0, combat=0, gender=0, is_tall=0, is_short=0, is_weight=0, is_light=0, is_bald=0, eye_color=0, publisher=0, place_of_birth=0, image=0)
...
for i in range(len(data)):
    ...
    # MALE = 0
    # FEMALE = 1
    data['gender'][i] = 0 if data.appearance[i]['gender'] == 'Male' else 1

    # YES = 1
    # NO = 0
    # NOT DEFINED = -1

    print(data.appearance[i]['height'][0])
    ...
    data['is_weight'][i] = 1 if lb >= 220 else -1 if lb == 0 else 0
